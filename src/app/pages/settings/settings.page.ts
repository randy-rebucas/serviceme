...
export class SettingsPage implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.classifications$ = new BehaviorSubject(null);

    this.subs.sink = combineLatest([
      this.availablities$,
      this.defaultCurrency$,
      this.classifications$
    ]).subscribe(([availablities, defaultCurrency, classifications]) => {
  ...
  ngOnInit() {
    this.subs.sink = from(this.authService.getCurrentUser()).pipe(
      ...
      this.classifications$.next(settings.classification);
    });
  ...
  onSelectClassification(event: CustomEvent) {
    this.classifications$.next(event.detail.value);
  }
